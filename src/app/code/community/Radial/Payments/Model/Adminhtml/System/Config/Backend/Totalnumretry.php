...
class Radial_Payments_Model_Adminhtml_System_Config_Backend_Totalnumretry extends Mage_Core_Model_Config_Data
{
	...
	$maxretries = Mage::helper('radial_core/payments/maxretries');
        ...
        $pendingCreditMemoSize = Mage::getModel('sales/order_creditmemo')->getCollection()
                        ->addFieldToFilter('state', Mage_Sales_Model_Order_Creditmemo::STATE_OPEN)->getSize();

        $pendingInvoicesSize = Mage::getModel('sales/order_invoice')->getCollection()
                                        ->addFieldToFilter('state', Mage_Sales_Model_Order_Invoice::STATE_OPEN)->getSize();

        $objectCollectionSize = $pendingCreditMemoSize + $pendingInvoicesSize;	
	
	$publicDisplay = '# of Payment Messages Waiting For Transmission Retry: '. $objectCollectionSize;
