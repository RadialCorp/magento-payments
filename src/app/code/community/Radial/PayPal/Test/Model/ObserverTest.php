...
class Radial_Paypal_Test_Model_ObserverTest extends Radial_Core_Test_Base
{
    public function testRollbackExpressPayment()
    {
        ...
        $voidModel = $this->getModelMock('radial_paypal/void');
        $voidModel->expects($this->once())
            ->method('void')
            ->will($this->returnSelf());
        $this->replaceByMock('model', 'radial_paypal/void', $voidModel);
        Mage::getModel('radial_paypal/observer')
     ...
     * And pass it down to the radial_paypal/multishipping::initializePaypalExpressCheckout()
     * method.
     */
    public function testHandleControllerActionPredispatch()
    {
        ...
        $multiShipping = $this->getModelMock('radial_paypal/multishipping', ['initializePaypalExpressCheckout']);
        ...
        /** @var Radial_PayPal_Model_Multishipping */
        $observer = Mage::getModel('radial_paypal/observer', ['multi_shipping' => $multiShipping]);
     ...
     * And pass it down to the radial_paypal/multishipping::processPaypalExpressPayment()
     * method.
     */
    public function testHandleEbayEnterpriseMultishippingBeforeSubmitOrderCreate()
    {
        ...
        /** @var Radial_PayPal_Model_Multishipping */
        $multiShipping = $this->getModelMock('radial_paypal/multishipping', ['processPaypalExpressPayment']);
        $multiShipping->expects($this->once())
