...
class Radial_PayPal_Test_Model_Express_ApiTest extends Radial_Core_Test_Base
{
    ...
    public function setUp()
    {
        ...
        $this->coreUrl->expects($this->any())
            ->method('getUrl')->will(
                $this->returnValueMap(
                    [
                        ['radial_paypal_express/checkout/return', [], 'the.return.url'],
                        ['radial_paypal_express/checkout/cancel', [], 'the.cancel.url'],
                    ]
                )
            );
        // stub sdk
     ...
     *   throw Radial_PayPal_Exception with a translated message.
    ...
    public function testSetExpressCheckout($isSuccessful)
    {
        ...
        if (!$isSuccessful) {
            $message
                = Radial_PayPal_Model_Express_Api::RADIAL_PAYPAL_API_FAILED;
            $this->setExpectedException('Radial_PayPal_Exception', $message);
        }
     ...
     * - the expected array structure is returned
     * - when the request was unsuccessful throw Radial_PayPal_Exception
     * @loadExpectation messageReplies
     * @dataProvider provideTrueFalse
     */
    public function testGetExpressCheckout($isSuccessful)
    {
        ...
        $this->sdk->expects($this->any())
            ...
            ->will($this->returnValue($request));
        // test setExpressCheckout
        $api = $this->getModelMock('radial_paypal/express_api', ['sendRequest']);
        $api->expects($this->any())
            ->method('sendRequest')
            ->will($this->returnValue($reply));
        EcomDev_Utils_Reflection::setRestrictedPropertyValues(
    ...
    public function testSendRequest()
    {
        ...
        $api = Mage::getModel('radial_paypal/express_api');
        $this->assertSame(
            $replyPayload,
    ...
    public function provideSdkExceptions()
    {
        ...
        $paypalException = 'Radial_PayPal_Exception';
        return [
    ...
    public function testSendRequestWithSdkException($exceptionType, $expectedExceptionType)
    {
        ...
        $api = Mage::getModel('radial_paypal/express_api', ['log_context' => $logContext]);
     ...
     * - throws an Radial_PayPal_Exception when the message fails
     * @loadExpectation messageReplies
     * @dataProvider provideTrueFalse
     */
    ...
    public function testAddLineItems()
    {
        ...
        $request = $this->getMockBuilder(self::SETEXPRESS_REQUEST_PAYLOAD)
            ->getMockForAbstractClass();
        $api = $this->getModelMockBuilder('radial_paypal/express_api')
