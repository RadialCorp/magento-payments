 ...
 */
class Radial_Paypal_Model_Observer
{
    /** @var Radial_PayPal_Model_Multishipping */
    protected $multiShipping;

    /**
     * @param array $initParams May have this key:
     *                          - 'multi_shipping' => Radial_PayPal_Model_Multishipping
     */
    public function __construct(array $initParams=[])
    {
        list($this->multiShipping) = $this->checkTypes(
            $this->nullCoalesce($initParams, 'multi_shipping', Mage::getModel('radial_paypal/multishipping'))
        );
    }
     ...
     *
     * @param  Radial_PayPal_Model_Multishipping
     * @return array
     */
    protected function checkTypes(Radial_PayPal_Model_Multishipping $multiShipping)
    {
    ...
    public function handleOrderCreatePaymentEvent(Varien_Event_Observer $observer)
    {
        ...
        Mage::getModel('radial_paypal/order_create_payment')
    ...
    public function handleOrderCreateContextEvent(Varien_Event_Observer $observer)
    {
        ...
        Mage::getModel('radial_paypal/order_create_context')
            ->updateOrderContext($order, $orderContext);
        return $this;
    }

    /**
     * @return Radial_PayPal_Model_Void
     */
    protected function _getVoidModel()
    {
        return Mage::getModel('radial_paypal/void');
    }

    /**
     * Listen to the 'controller_action_predispatch_checkout_multishipping_overview' event
     * in order to get the controller action and pass it down to the 'radial_paypal/multishipping::initializePaypalExpressCheckout()'
     * method.
     *
     ...
     * in order to get the sales/quote and pass it down to the 'radial_paypal/multishipping::processPaypalExpressPayment()'
     * method.
     *
     * @param  Varien_Event_Observer
     * @return self
