 ...
 */
class Radial_PayPal_Model_Express_Checkout
{
    const RADIAL_PAYPAL_ZERO_CHECKOUT_NOT_SUPPORTED = 'RADIAL_PAYPAL_ZERO_CHECKOUT_NOT_SUPPORTED';
    /**
     * Cache ID prefix for "pal" lookup
     *
     * @var string
     */
    const EBAYENTERPRISE_PAL_CACHE_ID = 'radial_paypal_express_checkout_pal';
    ...
    /** @var Radial_PayPal_Helper_Region */
        ...
        list($this->_helper, $this->_logger, $this->_config, $this->_quote, $this->_context, $this->regionHelper)
            = $this->_checkTypes(
                ...
                $this->_nullCoalesce(
                    $initParams,
                    'region_helper',
                    Mage::helper('radial_paypal/region')
                )
            );
        if (!$this->_quote) {
     ...
     * @param Radial_PayPal_Helper_Region
     *
     * @return array
     */
    protected function _checkTypes(
        ...
        Radial_PayPal_Helper_Region $regionHelper
    ) {
    ...
    public function start($returnUrl, $cancelUrl, $button = null)
    {
        ...
        if (!$this->_quote->getGrandTotal()
            ...
            Mage::throwException(
                $this->_helper->__(
                    self::RADIAL_PAYPAL_ZERO_CHECKOUT_NOT_SUPPORTED
                )
            );
        }
	...
	{	
        ...
        // to be created. In any such cases, the payment auth needs to be voided.
            ...
            throw Mage::exception('Radial_PayPal', $this->_helper->__(Radial_Paypal_Model_Express_Api::RADIAL_PAYPAL_API_FAILED));
        }
        $this->_quote->save();
    ...
    /**
     * @return Radial_Paypal_Model_Express_Api
     */
    protected function _getApi()
    {
        if (!$this->_api) {
            $this->_api = Mage::getModel('radial_paypal/express_api');
        }
        return $this->_api;
    }
